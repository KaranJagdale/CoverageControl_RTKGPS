...
def pub():
   # global Lv
    ...
    pub = rospy.Publisher('pwmCmd2',PwmInput,queue_size = 10)   
    #print(bot_loc)
    partition = voronoi(pos_grid, N_bots, 2, bot_loc)
    #print(partition)
...
def callbackVICON(data,args):
	...
	global w_prev
        ...
        if wantData:
                ...
                if args==BotNumber:
			if(allData[1] and allData[0] and allData[3]):
                     		wantData = False
                        q_prev[0][0]=q[0][0]
                        q_prev[1][0]=q[1][0]
                        q_prev[2][0]=q[2][0]
			...
			vel = np.array(inv((S(q).transpose()*S(q)))*(S(q).transpose())*np.matrix(q_dot))
                        if (wantData == False):
                            pub()
			    allData[1],allData[3],allData[0] = False,False,False
                        wantData = True  
...
def main():
    ...
    rospy.Subscriber('encoderData2', encoderData, callback)
